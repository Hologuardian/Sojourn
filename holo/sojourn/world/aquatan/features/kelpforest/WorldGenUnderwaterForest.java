package holo.sojourn.world.aquatan.features.kelpforest;
...
public class WorldGenUnderwaterForest extends WorldGenerator
{
    public WorldGenUnderwaterForest(boolean par1)
    {
        super(par1);
    }
                ...
                for (j1 = par3 - b0; j1 <= par3 + b0 && flag; ++j1)
                {
                    for (k1 = par5 - b0; k1 <= par5 + b0 && flag; ++k1)
                    {
                        if (i1 >= 0 && i1 < 256)
                        {
                            ...
                            if (l1 != AquatanBlocks.aquatanFlowingWater.blockID && (block != null && !block.isLeaves(par1World, j1,  i1, k1)) && l1 != AquatanBlocks.aquatanWater.blockID)
                            {
                                flag = false;
                            }
                        }
                ...
                if (isValidSoil && par4 < 256 - l - 1)
                {
                            ...
                            for (int k2 = par5 - k1; k2 <= par5 + k1; ++k2)
                            {
                                ...
                                if (Math.abs(j2) != k1 || Math.abs(l2) != k1 || par2Random.nextInt(2) != 0 && j1 != 0)
                                {
                                    ...
                                    if (block == null || block.canBeReplacedByLeaves(par1World, l1, i2, k2)
                                            || block.blockID ==  AquatanBlocks.aquatanWater.blockID || block.blockID ==  AquatanBlocks.aquatanFlowingWater.blockID)
                                    {
                    ...
                    for (i2 = 0; i2 < l; ++i2)
                    {
                        ...
                        if (j1 == 0 || block == null || block.isLeaves(par1World, par3, par4 + i2, par5)
                                || block.blockID ==  AquatanBlocks.aquatanWater.blockID || block.blockID ==  AquatanBlocks.aquatanFlowingWater.blockID)
                        {
                            this.setBlockAndMetadata(par1World, par3, par4 + i2, par5, Block.obsidian.blockID, 0);
                        }
                    }
